...
void gpio_pulldown (int iMask)
	{
	diag_message (DIAG_GPIO, "gpio_pullup (0x%08x)", iMask);
	if ( gpio == NULL ) return;
	gpio[GPPUD]		 =	PI_PUD_DOWN;
	usleep(20);
	gpio[GPPUDCLK0]	 =	iMask;
	usleep(20);
	gpio[GPPUD]		 =	0;
...
31		N 		Overvoltage 	0: Overvoltage allowed
								1: Overvoltage disallowed
30		O 		OTP Program1 	0: OTP programming allowed
								1: OTP programming disallowed
29		Q 		OTP Read1 		0: OTP reading allowed
								1: OTP reading disallowed
26-28	uuu 	Unused 			Unused
25		W 		Warranty bit 	0: Warranty is intact
								1: Warranty has been voided by overclocking
24		u 		Unused 			Unused
23		F 		New flag 		1: new-style revision
								0: old-style revision
20-22	MMM 	Memory size 	0: 256MB
								1: 512MB
  								2: 1GB
								3: 2GB
								4: 4GB
...
0,
0,
0x800000 + (1 << 4) + 1 + (0 << 20) + (1 << 16), // 0002 	B 		1.0 		256MB 	Egoman
0x800000 + (1 << 4) + 1 + (0 << 20) + (1 << 16), // 0003 	B 		1.0 		256MB 	Egoman
0x800000 + (1 << 4) + 2 + (0 << 20) + (0 << 16), // 0004 	B 		2.0 		256MB 	Sony UK
0x800000 + (1 << 4) + 2 + (0 << 20) + (6 << 16), // 0005 	B 		2.0 		256MB 	Qisda
0x800000 + (1 << 4) + 2 + (0 << 20) + (1 << 16), // 0006 	B 		2.0 		256MB 	Egoman
0x800000 + (0 << 4) + 2 + (0 << 20) + (1 << 16), // 0007 	A 		2.0 		256MB 	Egoman
0x800000 + (0 << 4) + 2 + (0 << 20) + (0 << 16), // 0008 	A 		2.0 		256MB 	Sony UK
0x800000 + (0 << 4) + 2 + (0 << 20) + (6 << 16), // 0009 	A 		2.0 		256MB 	Qisda
0x800000 + (1 << 4) + 2 + (1 << 20) + (1 << 16), // 000d 	B 		2.0 		512MB 	Egoman
0x800000 + (1 << 4) + 2 + (1 << 20) + (0 << 16), // 000e 	B 		2.0 		512MB 	Sony UK
0x800000 + (1 << 4) + 2 + (1 << 20) + (1 << 16), // 000f 	B 		2.0 		512MB 	Egoman
0x800000 + (3 << 4) + 3 + (1 << 20) + (0 << 16), // 0010 	B+ 		1.2 		512MB 	Sony UK
0x800000 + (6 << 4) + 1 + (1 << 20) + (0 << 16), // 0011 	CM1 	1.0 		512MB 	Sony UK
0x800000 + (2 << 4) + 2 + (0 << 20) + (0 << 16), // 0012 	A+ 		1.1 		256MB 	Sony UK
0x800000 + (3 << 4) + 3 + (1 << 20) + (1 << 16), // 0013 	B+ 		1.2 		512MB 	Embest
0x800000 + (6 << 4) + 1 + (1 << 20) + (1 << 16), // 0014 	CM1 	1.0 		512MB 	Embest
0x800000 + (2 << 4) + 2 + (0 << 20) + (1 << 16), // 0015 	A+ 		1.1 		256MB/512MB 	Embest
    };

int gpio_revision_code (void)
	{
	...
	while ( fgets (sLine, sizeof (sLine), pfil) )
		...
		if ( strncmp (sLine, "Revision", 8) == 0 )
			...
			while ( *ps )
				{
                ch = *ps;
                if ( ( ch >= '0' ) && ( ch <= '9' ) ) iRev = 16 * iRev + ch - '0';
                else if ( ( ch >= 'A' ) && ( ch <= 'F' ) ) iRev = 16 * iRev + ch - 'A' + 10;
                else if ( ( ch >= 'a' ) && ( ch <= 'f' ) ) iRev = 16 * iRev + ch - 'a' + 10;
				++ps;
				}
			break;
	...
	fclose (pfil);
    if ( ( iRev >= 0 ) && ( iRev < sizeof (iOldRev) / sizeof (int) ) ) iRev = iOldRev[iRev];
	return	iRev;
	}

int gpio_revision (void)
    {
    int iRev = gpio_revision_code ();
    if ( iRev < 0 ) return iRev;
    return iRev & 0x0F;
    }
...
int gpio_model (void)
    {
    int iModel = gpio_revision_code ();
    if ( iModel < 0 ) return iModel;
    iModel = ( iModel >> 4 ) & 0xFF;
    return iModel;
    }

int gpio_processor (void)
    {
    int iProc = gpio_revision_code ();
    if ( iProc < 0 ) return iProc;
    return ( iProc >> 12 ) & 0x0F;
    }

int gpio_memory (void)
    {
    int iMem = gpio_revision_code ();
